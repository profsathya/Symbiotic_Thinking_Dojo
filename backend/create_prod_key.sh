...
if [ $# -eq 0 ]; then
    echo "Usage: ./create_prod_key.sh --email user@example.com [--name \"User Name\"]"
    echo ""
    echo "Options:"
    echo "  --email     Student email address (required)"
    echo "  --name      Student name (optional)"
    echo "  --budget    Token budget, default 5000000 (optional)"
    echo "  --expires   Expiration date YYYY-MM-DD (optional)"
    exit 1
fi
...
DATABASE_TYPE=postgres DATABASE_URL="$LOCAL_URL" python3 manage_keys.py create "$@"
